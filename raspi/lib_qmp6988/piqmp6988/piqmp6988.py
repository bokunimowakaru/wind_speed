...
###############################################################################
# 本ソースコードは、下記からダウンロードしたものを改変したものです。
# https://pypi.org/project/piqmp6988/
# piqmp6988-1.0.1
#
# 上記からの改変部分
# - piqmp6988.py を Raspberry Pi 5 で動作するように変更しました。
#
# 改変前の著作権は Akihisa ONODA が有します。
# 改変部分の著作権は 国野 亘 が有します。
#
...
class PiQmp6988():
    def __init__(self, config={}):
        ...
        data = self.qmp6988.read_i2c_block_data(self.config['address'], REG_CHIP_ID, CHIP_ID_LENGTH)

        # キャリブレーションデータを読み出す
        # len, data = self.pi.i2c_read_i2c_block_data(self.qmp6988, REG_COE, COE_LENGTH)
        data = self.qmp6988.read_i2c_block_data(self.config['address'], REG_COE, COE_LENGTH)
    ...
    def __pre_process(self):
        ...
        """
        # self.pi = pigpio.pi()
        # self.pi = self.qmp6988.i2c_open(1, self.config['address'])
        self.qmp6988 = smbus.SMBus(1)
    ...
    def __set_powermode(self, mode):
        ...
        data = self.qmp6988.read_i2c_block_data(self.config['address'], REG_CTRL_MEAS, CTRL_MEAS_LENGTH)
        data_len = len(data)
        if (data_len == CTRL_MEAS_LENGTH):
            value = data[0] & 0xFC
            
            value |= mode & 0x03
            # self.pi.i2c_write_i2c_block_data(self.qmp6988, REG_CTRL_MEAS, [value])
            self.qmp6988.write_i2c_block_data(self.config['address'], REG_CTRL_MEAS, [value])
            time.sleep(0.02)
    ...
    def __set_oversampling(self, mode, sampling):
        ...
        #len, data = self.qmp6988.i2c_read_i2c_block_data(self.qmp6988, REG_CTRL_MEAS, CTRL_MEAS_LENGTH)
        data = self.qmp6988.read_i2c_block_data(self.config['address'], REG_CTRL_MEAS, CTRL_MEAS_LENGTH)
        data_len = len(data)
        if (data_len == CTRL_MEAS_LENGTH):
            value = data[0] & mask
            ...
            #self.qmp6988.i2c_write_i2c_block_data(self.qmp6988, REG_CTRL_MEAS, [value])
            self.qmp6988.write_i2c_block_data(self.config['address'], REG_CTRL_MEAS, [value])
            time.sleep(0.02)
    ...
    def __set_filter(self, filter):
        ...
        """
        value = filter & 0x07
        # self.qmp6988.i2c_write_i2c_block_data(self.qmp6988, REG_IIR_CNT, [value])
        self.qmp6988.write_i2c_block_data(self.config['address'], REG_CTRL_MEAS, [value])
        time.sleep(0.02)
    ...
    def read(self):
        ...
        # len, data = self.qmp6988.i2c_read_i2c_block_data(self.qmp6988, REG_DATA, DATA_LENGTH)
        data = self.qmp6988.read_i2c_block_data(self.config['address'], REG_DATA, DATA_LENGTH)
        data_len = len(data)
        self.__post_process()
